...
# Methylation values
meth <- fread(paste0("/scratch/prj/bell/epigenetics/Analysis/subprojects/juan/EPIC_data/NSHD/processed/chunks/beta_norm_QCed_99_chunk_", num, ".gz"), data.table = FALSE)
dim(meth)
rownames(meth) <- meth[,1]
meth <- meth[,-1]

# Info files
info <- read.csv("/scratch/prj/bell/epigenetics/Analysis/subprojects/juan/EPIC_data/NSHD/NSHD99_trueID_cleaned_unique_genoavail_JCF.csv.gz")
...
write.csv(result_all, paste("/scratch/prj/dtr/Groups_WorkSpace/JordanaBell/epigenetics/Analysis/subprojects/sergio/UK_cohorts_meQTL/11_cell_interaction/1_regression/NSHD/residuals_Smoking_Plate_Position_Cellprop-Mono_Sex_EPIC_ssnoob99_for1000GP3V5.txt_", num, sep = ""), quote = F, col.names = T, row.names = T)
