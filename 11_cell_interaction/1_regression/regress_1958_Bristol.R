...
# Methylation values
meth <- fread("/scratch/prj/bell/epigenetics/Data/EPIC/blood/other/Bristol/processed/beta_enmix.csv", data.table = FALSE)
dim(meth)
rownames(meth) <- meth[,1]
meth <- meth[,-1]

# Info files
info <- read.csv("/scratch/prj/bell/epigenetics/Analysis/subprojects/juan/meQTLs/1958/infobristol.csv")
...
write.csv(result, "/scratch/prj/dtr/Groups_WorkSpace/JordanaBell/epigenetics/Analysis/subprojects/sergio/UK_cohorts_meQTL/11_cell_interaction/1_regression/1958/residuals_Smoking_Plate_Position_Cellprop-Mono_Sex_EPIC_ssnoobBristol.csv", quote = F, col.names = T, row.names = T)
