...
doOne <- function(x){
  ...
  results2 <- summary(lmer(
    y ~ 1 + (1|Sentrix_ID) + (1|Sentrix_Position) + Age + PlasmaBlast + 
      CD8pCD28nCD45RAn + CD8.naive + CD4T + NK + Mono + Gran + Smoking, 
    data = info))$residuals
...
write.csv(result, "/mnt/lustre/groups/bell/epigenetics/Analysis/subprojects/sergio/UK_cohorts_meQTL/1_regression/TwinsUK/residuals_Plate_Position_Age_Smoking_Cellprop_EPIC_ssnoob.txt", quote = F, col.names = T, row.names = T)
