...
zz=gzfile('cis_meQTL_full.txt.gz','rt')  
dat=read.table(zz,header=T, stringsAsFactors = FALSE)
...
write.table(geno.map, "Blood.esi", sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
...
write.table(probes.tmp, "Blood.epi", sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
...
for(each in unique(dat$cpg)){
	...
	write.table(res, paste("mQTLResults/", each, ".esd", sep = ""), row.names = FALSE, quote = FALSE, sep = "\t")
}

probes<-unique(dat$cpg)
fileRef<-cbind(probes.tmp[match(probes, probes.tmp$name),], paste("mQTLResults/", probes, ".esd", sep = ""))
...
write.table(fileRef, "Blood.flist", sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)	
