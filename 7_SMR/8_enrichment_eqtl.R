...
# Set of associations
cis <- fread(paste0(path,"7_SMR/Results/SMReQTL/merge_results/SMReQTL_filter_Bonf_0.05_annotated.txt"),
             select = c("probeID", "ProbeChr", "Probe_bp"),
             header = TRUE, data.table = FALSE)
trans <- fread(paste0(path,"7_SMR/Results/SMReQTL_trans/merge_results/SMReQTL_trans_filter_Bonf_0.05_annotated.txt"),
               select = c("probeID", "ProbeChr", "Probe_bp"),
               header = TRUE, data.table = FALSE)
...
universe_cis <- fread(paste0(path,"6_annotation/meQTL_annotation/cis_top_cpg.txt"),
                      ...
                      header = TRUE, data.table = FALSE)
universe_trans <- fread(paste0(path,"6_annotation/meQTL_annotation/trans_top_cpg.txt"),
                        select = c("chr_cpg", "pos_cpg", "CpG", "chr_snp", "pos_snp", "top_SNP"),
                        header = TRUE, data.table = FALSE)
...
universe_cis_cpgs <- universe_cis %>%
  ...
  distinct()
universe_trans_cpgs <- universe_trans %>%
  select(chr_cpg, pos_cpg, CpG) %>%
  mutate(chr_cpg = paste0("chr",chr_cpg)) %>%
  distinct()
...
universe_trans_cpgs_gr <- makeGRangesFromDataFrame(universe_trans_cpgs,
                                                   ignore.strand = TRUE, seqnames.field = "chr_cpg", start.field = "pos_cpg",
                                                   end.field = "pos_cpg", keep.extra.columns = TRUE)
...
trans_cpgs_enrichment_lola <- runLOLA(trans_cpgs_gr, universe_trans_cpgs_gr, regionDB)
...
enrichment_pooled <- cis_cpgs_enrichment_lola %>%
  bind_rows(trans_cpgs_enrichment_lola) %>%
...
write.table(enrichment_pooled,
            paste0(path,"7_SMR/enrichment/enrichment_cpgs_smreqtl.txt"),
